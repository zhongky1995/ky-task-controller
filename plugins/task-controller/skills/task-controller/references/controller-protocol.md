...
Confirmed contract-impacting feedback or evidence is change-control input, not prose to absorb silently. Assess meaning before mutation: the keyword classifier is heuristic and can misread questions/negations or miss new evidence. Use read-only classification for inspection, and explicit `record-correction` when its result does not represent a confirmed change. Record actual impacts as `correctionEvents`; a callback containing events cannot pass, and open events block registration, gate, and completion. Revision must consume all open events (including an already ingested false positive) and may not start later than the earliest recommended invalid lane. Strict revision includes the complete replacement `contractSpec`. See `continuous-execution.md` for internal versus user decision boundaries.
